...
router.post('/login', isNotLoggedIn, (req, res, next) => {
  passport.authenticate('local', (err, user, info) => { //에러, 성공, 실패
    ...
    return req.login(user, async (err) => { // 세션에다 사용자 정보 저장 (어떻게? serializeUser)
      ...
      const fullUser = await db.User.findOne({//세션에 저장
        where: { id: user.id },
        attributes: ['id', 'email', 'nickname'],
        include: [{
          model: db.Post,
      ...
      return res.json(fullUser); //프론트에 사용자 정보 데이터 넘겨주기
    });
  })(req, res, next);
});

router.post('/logout', isLoggedIn, (req, res) => { // 실제 주소는 /user/logout
  if (req.isAuthenticated()) { //로그인 시 true
    req.logout();
    req.session.destroy(); //세션에 사용자 정보가 아닌 다른 정보가 있을 수 있으므로 destroy는 선택적으로 적용
